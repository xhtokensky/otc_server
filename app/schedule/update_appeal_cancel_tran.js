    ...
    async task(ctx) {
        try {
           ...
           /* let otcConf = await ctx.service.home.findOneOtcConf();
            ...
            let buyer_overtime = otcConf.appeal_overtime;

            //获取需要取消的订单
            let data = await ctx.service.order.queryOrderList(buyer_overtime,3);
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (!us) {
                    console.error('task >> update appeal cancel tran error ');
                    return
                }
            }
            ...
            console.error('task >> update appeal cancel tran error : ' + e.message);
            return;
        }
